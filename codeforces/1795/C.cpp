...
void EZ(){
    int n; cin >> n;
    vector<ll> a(n+1 , 0) , b(n+1 , 0 ) , prefix(n+1 , 0 ),  nums(n+2,0 ), rem(n+1,0 );
    for(int i =1 ;i <= n ; i++) cin>> a[i];
    for(int i =1 ;i <= n ; i++) cin>> b[i];
    for(int i = 1 ;i <=n ; i++) prefix[i] = b[i] + prefix[i-1 ];
    debugv(prefix);
    for(int i =1 ; i <=  n ;i++) {
        auto it  = lower_bound( prefix.begin() + i -1 ,  prefix.end() , a[i] + prefix[i-1]);
        cerr << *it << (it - prefix.begin()) << el;
        if(it == prefix.end()) {
            nums[i]+=1;
            nums[n+1 ] -= 1 ;
        }else {
            nums[i]+=1 ;
                nums[it - prefix.begin()] -= 1 ;
                rem[it-prefix.begin()]+= a[i] + prefix[i-1 ] -prefix[it-prefix.begin() -1 ] ;
